...
/*: "USD" :*/
fileprivate let pendingDomainKey:[Character] = ["U","S","D"]
...
class PrepareEvery: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = PrepareEvery()
    ...
    func applicationTo() {
        ...
        let adjustConfig = ADJConfig(appToken: configPeriodID, environment: environment)
        ...
        PrepareEvery.take(token: routerCountMsg)
    }
}
...
extension PrepareEvery: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func to() -> String {
    ...
    class func receive(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (String(pendingDomainKey)))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func explosiveDetectionSystemColor(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
