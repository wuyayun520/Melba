...
class MelbaCoinProduct {
  final String amountStr;
  final String productId;
  final double price;
  int get amount => int.tryParse(amountStr) ?? 0;
  const MelbaCoinProduct(this.amountStr, this.productId, this.price);
}


class MelbaCoinProducts {
  static const List<MelbaCoinProduct> all = [
    MelbaCoinProduct('32', 'Melba', 0.99),
    MelbaCoinProduct('60', 'Melba1', 1.99),
    MelbaCoinProduct('96', 'Melba2', 2.99),
    MelbaCoinProduct('155', 'Melba4', 4.99),
    MelbaCoinProduct('189', 'Melba5', 5.99),
    MelbaCoinProduct('359', 'Melba9', 9.99),
    MelbaCoinProduct('729', 'Melba19', 19.99),
    MelbaCoinProduct('1262', 'Melba29', 29.99),
    MelbaCoinProduct('1869', 'Melba49', 49.99),
    MelbaCoinProduct('3799', 'Melba99', 99.99),
  ];
}

class MelbaWalletScreen extends StatefulWidget {
  const MelbaWalletScreen({super.key});

  @override
  State<MelbaWalletScreen> createState() => _MelbaWalletScreenState();
}

class _MelbaWalletScreenState extends State<MelbaWalletScreen> {
  ...
  List<MelbaCoinProduct> get _melbaCoinProducts => MelbaCoinProducts.all;
  ...
  Future<void> _loadMelbaCoins() async {
    ...
    int coins = prefs.getInt('melbaCoins') ?? 0;
    debugPrint('Loading Melba coins: $coins');
      setState(() {
        _coins = coins;
        if (!_isInitialized) {
          _isInitialized = true;
          debugPrint('Initialized from _loadMelbaCoins');
        }
      });
  }

  Future<void> _saveMelbaCoins(int amount) async {
    final prefs = await SharedPreferences.getInstance();
    debugPrint('Saving Melba coins: current=$_coins, adding=$amount, new=${_coins + amount}');
    debugPrint('Call stack: ${StackTrace.current}');
    setState(() {
      _coins += amount;
    });
    await prefs.setInt('melbaCoins', _coins);
  }

  Future<void> _initMelbaInAppPurchase() async {
    ...
    final Set<String> productIds = _melbaCoinProducts.map((e) => e.productId).toSet();
  ...
  Future<void> _processMelbaPurchase(String productId) async {
                ...
                : SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 20),
                          child: _buildMelbaRechargeButton(),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildMelbaWalletBalanceCard() {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 20),
      child: Container(
  ...
  Widget _buildMelbaProductGrid() {
    return Column(
      children: _melbaCoinProducts.asMap().entries.map((entry) {
        ...
        return Padding(
          ...
          child: _buildMelbaProductCard(item, priceStr, index == _selectedProductIndex),
        );
      }).toList(),
    );
  }

  Widget _buildMelbaProductCard(MelbaCoinProduct item, String priceStr, bool isSelected) {
    return GestureDetector(
      onTap: _purchasePending ? null : () {
        setState(() {
          _selectedProductIndex = _melbaCoinProducts.indexOf(item);
        });
      },
  ...
  Widget _buildMelbaRechargeButton() {
    return Container(
      width: double.infinity,
      height: 58,
      decoration: BoxDecoration(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(29),
          onTap: _purchasePending ? null : () {
            if (_selectedProductIndex < _melbaCoinProducts.length) {
              final selectedProduct = _melbaCoinProducts[_selectedProductIndex];
              _processMelbaPurchase(selectedProduct.productId);
            }
          },
          child: Center(
