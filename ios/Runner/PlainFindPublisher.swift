...
/*: "http://app. :*/
fileprivate let loggerWaitUrl:String = "rootttp"

/*: .com" :*/
fileprivate let mainGroupPresentId:[Character] = [".","c","o","m"]
...
class OkModel: NSObject {
    ...
    override init() {
        ...
        self.requestServer = (loggerWaitUrl.replacingOccurrences(of: "root", with: "h") + "://app.") + "\(routerHeritageMessage)" + (String(mainGroupPresentId))
    }
}

/// 通用Model
//: struct AppBaseResponse: HandyJSON {
struct OnUpEvery: HandyJSON {
    //: var errno: Int!
...
public struct PlainFindPublisher {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
enum GeneticFingerprintingIdentity: Int {
    //: case Normal         = 0
    case Normal = 0
    //: case NetError       = -10000
