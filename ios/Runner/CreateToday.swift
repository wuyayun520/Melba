...
class CreateToday {
    ...
    static let shared = CreateToday()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func purchaseInManager(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func item(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func remove(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
